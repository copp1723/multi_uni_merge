...
class WebSocketService(BaseService):
            ...
            with self.app.app_context():
                ...
                try:
                    ...
                    if not self.mcp_filesystem_service:
                        ...
                        self.mcp_filesystem_service = getattr(
                            current_app, "mcp_filesystem_service", None
                        )

                    # Create agent service with MCP filesystem
                    if self.mcp_filesystem_service and agent_service:
                        set_mcp_filesystem_service(self.mcp_filesystem_service)
                        ...
                        if agent_service.mcp_filesystem:
                            ...
                            logger.error(
                                f"❌ Agent {agent_id} missing MCP filesystem access"
                            )
                    ...
                    if openrouter_service and agent_service:
                        ...
                        if supermemory_service:
                            try:
                                memory_context = (
                                    supermemory_service.get_cross_agent_context(
                                        message.content, exclude_agent=agent_id
                                    )
                                )
                                if memory_context:
                                    context_text = (
                                        supermemory_service.format_memory_context(
                                            memory_context
                                        )
                                    )
    ...
    def send_file_access_notification(
        self, client_id: str, operation: str, file_path: str, success: bool
    ):
...
class SwarmNamespace(Namespace):
    ...
    def on_connect(self):
        ...
        emit(
            "connection_confirmed",
            {
                "client_id": client_id,
                "timestamp": datetime.now(timezone.utc).isoformat(),
                "mcp_filesystem_available": bool(
                    self.websocket_service.mcp_filesystem_service
                ),
            },
        )
    ...
    def on_disconnect(self):
        ...
        if client_id in self.websocket_service.connected_clients:
            ...
            logger.info(
                f"🔌 Client disconnected: {client_id} (user: {user_info.get('user_id')})"
            )

        # Clean up any streaming sessions
        sessions_to_remove = [
            session_id
            for session_id, session in self.websocket_service.streaming_sessions.items()
